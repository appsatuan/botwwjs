...
app.post('/send-message-reset', async (req, res) => {
    ...
    try {
        ...
        const condition = (nis.length === 10) ? 'sidanira' : 'nis';
        const [rows] = await connection.execute(`SELECT phone FROM userbaru WHERE ${condition} = ?`, [nis]);
        if (rows.length === 0) {
            return res.status(404).send({ success: false, message: 'NIS not found' });
        }
        ...
        const message = `${greeting}, Silakan masukkan token *${token}* ke website register https://siswa.sman81.sch.id`;

        await connection.execute(`UPDATE userbaru SET token = ? WHERE ${condition} = ?`, [token, nis]);

        await axios.post('http://cbt.sman81.sch.id:8000/send-message', {
            number: phone,
            message: message
        });

        res.status(200).send({ success: true, message: 'Message sent successfully', password: password });
    } catch (error) {
